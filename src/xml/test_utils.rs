...
mod tests {
    ...
    fn test_cleanup_xml() {
        let input = r#"
            <Main>
                <SomeTag abc="def" />   <SomeTag abc="xyz" />
                <SomeTag abc="abc" >
                    <SomeSubTag abc="xyz"/>
                    <SomeSubTag abc="xyz"></SomeSubTag>
                </SomeTag>
            </Main>
        "#;

        let expected = r#"<Main><SomeTag abc="def"/><SomeTag abc="xyz"/><SomeTag abc="abc"><SomeSubTag abc="xyz"/><SomeSubTag abc="xyz"></SomeSubTag></SomeTag></Main>"#;

        assert_eq!(cleanup_xml(input.into()), expected);
    }
}